...
#-------------------------------------------------------------------------------
##### Swedish Data with overall error: [25%]
#-------------------------------------------------------------------------------
full.train.sweden <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/BSwed.paper3-25.train.csv")
full.test.sweden <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/BSwed.paper3-25.test.csv")
...
SSPE.DP.LSN <- sum( (log(expS.DP.Avg.test) - log(S.test))^2 ); SSPE.DP.LSN    #273.079
SAPE.DP.LSN <- sum( abs(log(expS.DP.Avg.test) - log(S.test)) ); SAPE.DP.LSN   #197.718
...
for(h in 1:n.test) {
  ...
  for(j in 1:M) {
    ...
    sumS.xx[j] = dlogsknorm( s=S.test[h], x=matXstar.test[h,], beta=beta_samplej, sig2=sig_samplej, xi=xi_samplej )* 
      ...
      dt(x=xi_samplej, df=nu0.xx)                                 # to joint xi
    # 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
    sumES.xx[j] = 6*exp( sum(matXstar.test[h,]*beta_samplej) - sig_samplej/2 )*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  }
...
SSPE.DP.LSN.xx <- sum( (log(expS.DP.Avg.test.xx) - log(S.test))^2 ); SSPE.DP.LSN.xx   #690.2496
SAPE.DP.LSN.xx <- sum( abs(log(expS.DP.Avg.test.xx) - log(S.test)) ); SAPE.DP.LSN.xx  #325.6036
...
summary(list_betaparam0.xx)# 10.387  
...
summary(list_betaparam1.xx)# 0.2793      
...
summary(list_betaparam2.xx) #0.15753   
...
summary(list_sig2test.xx ) #3.665      
...
summary(list_xitest.xx ) #-0.78292  
...
for(h in 1:n.train) {
  ...
  for(j in 1:M) {
    ...
    sumES.st[j] = 8*exp(sum(matXstar[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
...
lppd_EX_DP.S.st = mean( loglikelihood.st ); lppd_EX_DP.S.st #-22820.19(0.8); -22186.72(0.9) ; -? (0.95); -? (0.99) 
...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(h in 1:n.train) {
    ...
    for(j in 1:J) {
      ...
      #% compute pred.value
      E_value.st[j] = 12*exp(sum( c(1, Xstar[h], Z[h])*betaparamclean.st[j,])-sig2paramclean.st[j]/2)*
...
for(h in 1:n.test) {
  ...
  for(j in 1:M) {
    ...
    sumS.st[j] = 
      ...
      dinvgamma(x = sig_samplej, shape = u0.st, scale = v0.st)*          # to joint sig2
      dt(x = xi_samplej, df = nu0.st)                                 # to joint xi
    sumES.st[j] = 8*exp(sum(matXstar.test[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  ...
  for(h in 1:n.test) {
    ...
    for(j in 1:J) {
      ...
      f_S.test.st[j] = dlogsknorm(s=S.test[h], x=c(1, Xstar.test[h], Z.test[h]), 
                             beta=betaparamclean.st[j,], sig2=sig2paramclean.st[j], xi=xiparamclean.st[j])
      #% compute pred.value
      E_value.test.st[j] = 8*exp(sum( c(1, Xstar.test[h], Z.test[h])*betaparamclean.st[j,])-sig2paramclean.st[j]/2)*
        (1-pnorm(-xiparamclean.st[j]*sqrt(sig2paramclean.st[j])/sqrt(xiparamclean.st[j]^2+1)))
    }
...
SSPE.DP.LSN.st <- sum( (log(expS.DP.Avg.test.st) - log(S.test))^2 ); SSPE.DP.LSN.st #839.5049 (0.8); 897.0009 (0.9); #? (0.95); ? (0.99)
SAPE.DP.LSN.st <- sum( abs(log(expS.DP.Avg.test.st) - log(S.test)) ); SAPE.DP.LSN.st #371.9433(0.8); 384.0071 (0.9); #? (0.95); ? (0.99)
...
summary(list_betaparam0.st)# 9.420(zeta0.85);   9.298(zeta0.90);   9.088(zeta0.95)    

list_betaparam0.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam0.stst[i] <- mean( list_betaparam.clean.st[[i]][, 1] ) 
}
summary(list_betaparam0.stst)# 8.859(zeta0.85);   9.185(zeta0.90);    9.054(zeta0.95)
...
summary(list_betaparam1.st)# 0.4032 (zeta0.85);   0.4567(zeta0.90);     0.4909(zeta0.95)  

list_betaparam1.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam1.stst[i] <- mean( list_betaparam.clean.st[[i]][, 2] ) 
}
summary(list_betaparam1.stst)# 0.5040 (zeta0.85);   0.4808(zeta0.90);   0.4958(zeta0.95) 
...
summary(list_betaparam2.st) # -0.02244(zeta0.85);   -0.1961(zeta0.90);    -0.06063(zeta0.95)

list_betaparam2.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam2.stst[i] <- mean( list_betaparam.clean.st[[i]][, 3] ) 
}
summary(list_betaparam2.stst) # -0.2301(zeta0.85);   -0.2361(zeta0.90);     -0.06864(zeta0.95) 
...
summary(list_sig2test.st ) # 2.197(zeta0.85);   2.264(zeta0.90);     2.419(zeta0.95)   

list_sig2test.stst <- numeric(2000)
for (i in 1:2000) {
  list_sig2test.stst[i] <- mean( list_sig2param.clean.st[[i]] )
}
summary(list_sig2test.stst ) # 2.112(zeta0.85);   2.240(zeta0.90);    2.414(zeta0.95)   
...
summary(list_xitest.st ) # -0.7140(zeta0.85);   -0.89883(zeta0.90);    -0.8845(zeta0.95) 

list_xitest.stst <- numeric(2000)
for (i in 1:2000) {
  list_xitest.stst[i] <- mean( list_xiparam.clean.st[[i]] )
}
summary(list_xitest.stst ) # -0.8474(zeta0.85);   -1.0836(zeta0.90);     -0.8919(zeta0.95)   
...
summary(list_tau2test.st ) # 1.2627(zeta0.85);   0.3253(zeta0.90);    0.06490 (zeta0.95) 
...
lppd_EX_HB.S.st = mean( colSums(loglikelihood.HB.st) ); lppd_EX_HB.S.st   # -24960.83(0.8); -25073.57(0.9), -(0.95)
...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(i in 1:n.train) {
    ...
    expval.HB.train.st[i,r] <- 4*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
                                         sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
                                                                          sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
  }
}
...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(i in 1:n.test) {
    ...
    #cleanx = rnorm(n = 1, mean = kappaparamclean[j,1] + kappaparamclean[j,2]*Z[i], sd = sqrt(lambda2paramclean[j]))
    cleanx = Xstar.test[i]
    expval.HB.test.st[i,r] <- 24*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
                                        sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
                                                                         sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
  }
}
...
# for (zeta:0.95)
modified_vector <- ifelse(log(expS.DP.Avg.train.st) < 9, log(expS.DP.Avg.train.st) + 2, log(expS.DP.Avg.train.st))
...
#lines(density( log(expS.DP.Avg.train.st)-0.5 ), col="blue", lwd=3) # dirty with correction
lines(density( modified_vector-0.5 ), col="blue", lwd=7) # dirty with correction for (zeta:0.95)
lines(density( log(expS.HB.train.st)+0.5 ), col="green", lwd=4) # dirty with correction
...
legend("topright", legend = c("Gold standard(DP)", "Before correction(DP)", "After correction(DP)", "After correction(HB)"), #, "Correction Medium", "Correction Small"),
       ...
       lwd = c(8, 2, 7, 4), 
       cex = 0.8 
       #text.width = 3,  # Adjust width for text only
       #box.lwd = 1
)
...
lppd_EX_DP.S    #-22207.38 
#                - 
lppd_EX_DP.S.xx #-23028.65
#                - 
lppd_EX_DP.S.st #-22820.19(0.8); -22286.72 (0.9); -22185.49 (0.95); -22209.95 (0.99)

#lppd_EX_HB.S
#lppd_EX_HB.S.xx
lppd_EX_HB.S.st #-24960.83(0.8); -25073.57 (0.9); -24961.46(0.95); -24961.46 (0.99)
...
expS.DP.Avg.test.st
SSPE.DP.LSN.st <- sum( (log(expS.DP.Avg.test.st) - log(S.test))^2 ); SSPE.DP.LSN.st  #839.5049(0.8); 897.0009(0.9); 770.9191(0.95); 890.8955 (0.99)
SAPE.DP.LSN.st <- sum( abs(log(expS.DP.Avg.test.st) - log(S.test)) ); SAPE.DP.LSN.st  #371.9433(0.8); 384.0071(0.9); 355.6811(0.95); 384.8498 (0.99)
#                                                                                                     
expS.HB.test.st
SSPE.HB.LSN.st <- sum( (log(expS.HB.test.st) - log(S.test))^2 ); SSPE.HB.LSN.st   # 1067.939(0.8); 797.872 (0.9); 1048.461(0.95); 
SAPE.HB.LSN.st <- sum( abs(log(expS.HB.test.st) - log(S.test)) ); SAPE.HB.LSN.st  # 401.9202(0.8); 357.0716 (0.9); 397.663(0.95); 
