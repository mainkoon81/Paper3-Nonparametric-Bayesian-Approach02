...
# 
# 
# #% empirical X_bar ? using "aggregate(.)": investigation by splitting the data into subset
...
#   for (i in 1:n.train){
#     numerator = numerator + 
#       dlogsknorm_log(s=S[i], x=matX_tru[i,], beta=t(as.matrix(beta_prop_full)), sig2=sig2_prev_full, xi=xi_prev_full)
# 
#     denominator = denominator + 
#       dlogsknorm_log(s=S[i], x=matX_tru[i,], beta=t(as.matrix(beta_prev_full)), sig2=sig2_prev_full, xi=xi_prev_full)
#   }
#   ratio_beta = min(exp(numerator-denominator), 1)
#   
#   U = runif(n = 1, min = 0, max = 1)
...
#   for (i in 1:n.train){
#     numerator = numerator + 
#       dlogsknorm_log(s=S[i], x=matX_tru[i,], beta=t(as.matrix(betaparam_full)), sig2=sig2_prop_full, xi=xi_prev_full)
#       
#     denominator = denominator + 
#       dlogsknorm_log(s=S[i], x=matX_tru[i,], beta=t(as.matrix(betaparam_full)), sig2=sig2_prev_full, xi=xi_prev_full)
#   }
#   ratio_sig2 = min(exp(numerator-denominator), 1)
#   
#   U = runif(n = 1, min = 0, max = 1)
...
#   for (i in 1:n.train){
#     numerator = numerator + 
#       dlogsknorm_log(S[i], matX_tru[i,], t(as.matrix(betaparam_full)), sig2param_full, xi_prop_full)
#     
#     denominator = denominator + 
#       dlogsknorm_log(S[i], matX_tru[i,], t(as.matrix(betaparam_full)), sig2param_full, xi_prev_full)
#   }
#   # compute the ratio
#   ratio_xi = min(exp(numerator-denominator), 1)
#   
...
#     X_j = X_tru[cl_membership==j] #covariate
...
#     matX_j = matX_tru[cl_membership==j, ] #covariate
#     
#     
...
#   
#   for(i in 1:n.train) {
#     x_i = matX_tru[i, ]      # first row..{1,x,z}
...
#     expval.train[i,r] <- 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
#                                   sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
#                                                                     sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
#   }
# }
# 
# expS.HB.train <- apply(X=expval.train, MARGIN=1, FUN=mean)
# 
# summary(S)
# summary(expS.HB.train)
# 
# format( aggregate(log(S), by=list(cl_membership), FUN=summary), scientific=FALSE)
# format( aggregate(log(expS.HB.train), by=list(cl_membership), FUN=summary), scientific=FALSE)
# 
# lppd_EX_S.S <- mean( colSums(loglikelihood) ); lppd_EX_S.S # -23375.66
# 
...
#   lines(density(log(expS.HB.train)[cl_membership==j]), col="red", ylim=c(0,1.6), lwd=2)
# }
# 
# 
# ###> Testing data
...
#     #cleanx = rnorm(n = 1, mean = kappaparamclean[j,1] + kappaparamclean[j,2]*Z[i], sd = sqrt(lambda2paramclean[j]))
#     cleanx = X_tru.test[i]
#     expval.test[i,r] <- 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
#                                    sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
#                                                                     sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
#   }
# }
# expS.HB.test <- apply(X=expval.test, MARGIN=1, FUN=mean)
# 
# summary(S.test)
# summary(expS.HB.test)
# 
# format( aggregate(log(S.test), by=list(cl_membership.test), FUN=summary), scientific=FALSE)
# format( aggregate(log(expS.HB.test), by=list(cl_membership.test), FUN=summary), scientific=FALSE)
# 
# 
# # Clusterwise predictive density plots
# par(mfrow = c(2,3))
...
#        xlab = paste("Cluster", j))
#   lines(density(log(expS.HB.test)[cl_membership.test==j]), col="red", ylim=c(0,1.6), lwd=2)
# }
# 
# 
...
for (r in 1:total_iter) {
  ...
  for(j in 1:J) {
    ...
    xiparamclean[j] = sign(xiparam.st[j])*sqrt( xiparam.st[j]^2*(betaparamclean[j,2]^2/sig2paramclean[j]+1/tau2param[j]+1/lambda2paramclean[j])/
...
lppd_EX_HB.S.st = mean( colSums(loglikelihood.st) ); lppd_EX_HB.S.st
# -23258.21(0.9),           
...
format( aggregate(log(S), by=list(cl_membership), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.HB.train.st), by=list(cl_membership), FUN=summary), scientific=FALSE)

lppd_EX_HB.S.st <- mean( colSums(loglikelihood.st) ); lppd_EX_HB.S.st # -23377.5
...
for(j in 1:J) {
  hist(log(S)[cl_membership==j], freq=FALSE, breaks=50, col="white", xlim=c(5,15), ylim=c(0,1.6),
       xlab = paste("Cluster", j))
  lines(density(log(expS.HB.train.st)[cl_membership==j]), col="red", ylim=c(0,1.6), lwd=2)
}
...
format( aggregate(log(S.test), by=list(cl_membership.test), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.HB.test.st), by=list(cl_membership.test), FUN=summary), scientific=FALSE)
...
for(j in 1:J) {
  hist(log(S.test)[cl_membership.test==j], freq=FALSE, breaks=50, col="white", xlim=c(5,15), ylim=c(0,1.6),
       xlab = paste("Cluster", j))
  lines(density(log(expS.HB.test.st)[cl_membership.test==j]), col="red", ylim=c(0,1.6), lwd=2)
}
...
for(j in 1:J) {
  hist(log(S)[cl_membership==j], freq=FALSE, breaks=50, col="white", xlim=c(5,15), ylim=c(0, 2.8),
       xlab = paste("Cluster", j))
  #lines(density(log(expS.HB.train)[cl_membership==j]), col="red", lwd=2)     # clean 
  #lines(density(log(EX_S.train.dirty)[cl_membershipS==j]), col="black", lty = "dotted") # dirty
  lines(density(log(expS.HB.train.st)[cl_membership==j]), col="blue", lwd=1) # dirty with correction
...
lppd_EX_HB.S.st  #-23258.21

###> SSSE
# #> The model
# expS.HB.test
# SSPE.HB.LSN <- sum( (log(expS.HB.test) - log(S.test))^2 ); SSPE.LSN   #310.2905
# SAPE.HB.LSN <- sum( abs(log(expS.HB.test) - log(S.test)) ); SAPE.LSN  #213.1511

#> Gustafson Correction
expS.HB.test.st
SSPE.HB.LSN.st <- sum( (log(expS.HB.test.st) - log(S.test))^2 ); SSPE.HB.LSN.st   #263.7503
SAPE.HB.LSN.st <- sum( abs(log(expS.HB.test.st) - log(S.test)) ); SAPE.HB.LSN.st  #201.9686
...
alalpha = 0.5
alalpha = 0.1
#quantile_threshold <- quantile(expS.HB.train, alalpha); quantile_threshold
...
pdf <- density(expS.DP.train)$y
#pdf.HB <- density(expS.HB.train)$y
pdf.DP.xx <- density(expS.DP.train.xx)$y
pdf.DP.st <- density(expS.DP.train.st)$y
