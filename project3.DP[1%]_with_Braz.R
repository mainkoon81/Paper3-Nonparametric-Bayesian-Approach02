...
#-------------------------------------------------------------------------------
##### Brazilian Data with overall error: [1%]
#-------------------------------------------------------------------------------
full.train.brazil <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/Brazil.paper3-1.train.csv")
full.test.brazil <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/Brazil.paper3-1.test.csv")
...
for(h in 1:n.train) {
  ...
  for(j in 1:M) {
    ...
    sumES.st[j] = 12*exp(sum(matXstar[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
...
for(h in 1:n.test) {
  ...
  for(j in 1:M) {
    ...
    sumS.st[j] = 
      ...
      dinvgamma(x = sig_samplej, shape = u0.st, scale = v0.st)*          # to joint sig2
      dt(x = xi_samplej, df = nu0.st)                                 # to joint xi
    sumES.st[j] = 12*exp(sum(matXstar.test[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
