...
#-------------------------------------------------------------------------------
##### Swedish Data with overall error: [10%]
#-------------------------------------------------------------------------------
full.train.sweden <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/BSwed.paper3-10.train.csv")
full.test.sweden <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/BSwed.paper3-10.test.csv")
...
summary(list_betaparam0.xx)# 8.069   
...
summary(list_betaparam1.xx)# 0.7279    
...
summary(list_betaparam2.xx) #-0.8047 
...
summary(list_sig2test.xx ) #2.996   
...
lppd_EX_DP.S.st = mean( loglikelihood.st ); lppd_EX_DP.S.st #-25820.42 (0.9) ;-22818.55 (0.95); -21818.11 (0.99) 
...
SSPE.DP.LSN.st <- sum( (log(expS.DP.Avg.test.st) - log(S.test))^2 ); SSPE.DP.LSN.st #346.7241 (0.9); #356.4714 (0.95); 364.8085 (0.99)
SAPE.DP.LSN.st <- sum( abs(log(expS.DP.Avg.test.st) - log(S.test)) ); SAPE.DP.LSN.st #230.2235 (0.9); #231.9931 (0.95); 234.3397 (0.99)
...
summary(list_betaparam0.st)# 8.074(zeta0.85);  7.953(zeta0.9); 7.932(zeta0.95)  

list_betaparam0.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam0.stst[i] <- mean( list_betaparam.clean.st[[i]][, 1] ) 
}
summary(list_betaparam0.stst)# 7.657(zeta0.85); 7.774(zeta0.9); 7.897(zeta0.95) 
...
summary(list_betaparam1.st)# 0.7388(zeta0.85);   0.7165(zeta0.9); 0.7463(zeta0.95)    

list_betaparam1.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam1.stst[i] <- mean( list_betaparam.clean.st[[i]][, 2] ) 
}
summary(list_betaparam1.stst)# 0.8209(zeta0.85); 0.7542(zeta0.9); 0.7538(zeta0.95)    
...
summary(list_betaparam2.st) #-0.9288(zeta0.85); -0.7915(zeta0.9); -0.9301(zeta0.95)

list_betaparam2.stst <- numeric(2000)
for (i in 1:2000){
  list_betaparam2.stst[i] <- mean( list_betaparam.clean.st[[i]][, 3] ) 
}
summary(list_betaparam2.stst) #-1.0086(zeta0.85); -0.8265(zeta0.9); -0.9371(zeta0.95)
...
summary(list_sig2test.st ) #2.032(zeta0.85); 1.675(zeta0.9); 1.758(zeta0.95)       

list_sig2test.stst <- numeric(2000)
for (i in 1:2000) {
  list_sig2test.stst[i] <- mean( list_sig2param.clean.st[[i]] )
}
summary(list_sig2test.stst ) #1.822(zeta0.85); 1.578(zeta0.9);  1.738(zeta0.95)        
...
summary(list_xitest.st ) #0.6981(zeta0.85); 0.15377(zeta0.9);  0.1138(zeta0.95)     

list_xitest.stst <- numeric(2000)
for (i in 1:2000) {
  list_xitest.stst[i] <- mean( list_xiparam.clean.st[[i]] )
}
summary(list_xitest.stst ) #-0.02534(zeta0.85); 1.9905(zeta0.9);  0.33886(zeta0.95)      
...
summary(list_tau2test.st ) #0.4173(zeta0.85); 0.2167(zeta0.9);  0.03971(zeta0.95)   
...
lppd_EX_HB.S.st = mean( colSums(loglikelihood.HB.st) ); lppd_EX_HB.S.st   # -23436.2(0.9), -23439.83(0.95)
...
lines(density( log_expS.DP.Avg.train ), col="red", lwd=8)
...
lines(density( log(expS.HB.train.st)-0.4 ), col="green", lwd=4) # dirty with correction
...
lppd_EX_DP.S    #-22207.38 
#                - 
lppd_EX_DP.S.xx #-22238.93
#                - 
lppd_EX_DP.S.st #-25820.42 (0.9); -22818.55 (0.95); -21818.11 (0.99)

#lppd_EX_HB.S
#lppd_EX_HB.S.xx
lppd_EX_HB.S.st #-23436.2 (0.9); -23439.83(0.95); 
...
##> Modelrisk
expS.DP.Avg.test.xx
SSPE.DP.LSN.xx <- sum( (log(expS.DP.Avg.test.xx) - log(S.test))^2 ); SSPE.DP.LSN.xx   #371.0732
SAPE.DP.LSN.xx <- sum( abs(log(expS.DP.Avg.test.xx) - log(S.test)) ); SAPE.DP.LSN.xx  #238.8172
#expS.HB.test.xx
...
#                                                                                                     
expS.HB.test.st
SSPE.HB.LSN.st <- sum( (log(expS.HB.test.st) - log(S.test))^2 ); SSPE.HB.LSN.st   # 375.0629 (0.9); 386.6036(0.95); 
SAPE.HB.LSN.st <- sum( abs(log(expS.HB.test.st) - log(S.test)) ); SAPE.HB.LSN.st  # 235.1058 (0.9); 238.2918(0.95); 
