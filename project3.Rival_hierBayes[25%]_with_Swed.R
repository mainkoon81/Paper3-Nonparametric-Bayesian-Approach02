...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(i in 1:n.train) {
    ...
    #                              sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
    #                                                               sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
    expval.train.st[i,r] <- 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
                                      sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
                                                                       sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
  }
}
...
lppd_EX_S.S.st <- mean( colSums(loglikelihood.st) ); lppd_EX_S.S.st # -23587.85
...
for(r in 1:(total_iter-r_convergence)) {
  ...
  for(i in 1:n.test) {
    ...
    #cleanx = rnorm(n = 1, mean = kappaparamclean[j,1] + kappaparamclean[j,2]*Z[i], sd = sqrt(lambda2paramclean[j]))
    cleanx = Xstar.test[i]
    expval.test.st[i,r] <- 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - 
                                     sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*
                                                                      sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
  }
}
...
lppd_EX_S.S   #-23656.28
lppd_EX_S.S.st#-23587.85


###> SSSE
#> The model
expS.test
SSPE.LSN <- sum( (log(expS.test) - log(S.test))^2 ); SSPE.LSN   #516.4704
SAPE.LSN <- sum( abs(log(expS.test) - log(S.test)) ); SAPE.LSN  #278.9308

#> Gustafson Correction
expS.test.st
SSPE.LSN.st <- sum( (log(expS.test.st) - log(S.test))^2 ); SSPE.LSN.st   #383.6682
SAPE.LSN.st <- sum( abs(log(expS.test.st) - log(S.test)) ); SAPE.LSN.st  #237.7065
