...
#-------------------------------------------------------------------------------
##### Brazilian Data with overall error: [25%]
#-------------------------------------------------------------------------------
full.train.brazil <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/Brazil.paper3-25.train.csv")
full.test.brazil <- read.csv("C:/Users/kimm4/Desktop/WORKSPACE/2nd 3rd paper/DATASET/Brazil.paper3-25.test.csv")
#full.train.brazil <- read.csv("Brazil.paper3-1.train.csv")
#full.test.brazil <- read.csv("Brazil.paper3-1.test.csv")
head(full.train.brazil)
str(full.train.brazil)
summary(full.train.brazil)

n.train <- nrow(full.train.brazil)
n.test <- nrow(full.test.brazil)

#%%%% test %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
##### Variable Definitions
S.test = full.test.brazil$AggClaim
logS.test = log(S.test)
Z.test = full.test.brazil$Affiliation

# dirty data on log scale, divided by standard dev to be on same scale
Xstar.test = full.test.brazil$ln_Expos_err
X_tru.test = full.test.brazil$ln_Expos
...
n.breaks.test = sqrt( nrow(full.test.brazil) ) #******Rule of thumb
...
###>>>>> littel changes....
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# re-define initialization of cl-membership - train, test all together!!!!!!!!!
df = rbind(full.train.brazil, full.test.brazil); df 
nrow(df)
J=4
set.seed(1)
lnS = log( df$AggClaim ) 
...
cluster = kmeans(cbind(lnS,X_tru,Z), J)
cl_membership = cluster$cluster
table(cl_membership)


# for GS model
cl_membership.train = cl_membership[1:n.train]
cl_membership.test = cl_membership[(n.train+1):(n.train+n.test)]

# cl_membership.train.xx = cl_membership[1:n.train]
# cl_membership.test.xx = cl_membership[(n.train+1):(n.train+n.test)]
# 
# cl_membership.train.st = cl_membership[1:n.train]
# cl_membership.test.st = cl_membership[(n.train+1):(n.train+n.test)]


###>>> back to normal
#%%%% train %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%#
##### Variable Definitions
S = full.train.brazil$AggClaim 
logS = log(S)
Z = full.train.brazil$Affiliation

# dirty data on log scale, divided by standard dev to be on same scale
# Xstar = full.train.brazil$ln_insured_err/sd(full.train.brazil$ln_Expos_err) 
Xstar = full.train.brazil$ln_Expos_err
X_tru = full.train.brazil$ln_Expos 
# or...X on log scale, divided by standard dev to be on same scale
#X = log(full.train.brazil$SumInsAvg) / sd( log(full.train.brazil$SumInsAvg) )  

matXstar = as.matrix(cbind(1, Xstar, Z))
matX_tru = as.matrix(cbind(1, X_tru, Z))
#matX = as.matrix(cbind(1, X, Z))

n.breaks.train = sqrt( nrow(full.train.brazil) ) #******Rule of thumb
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot(density(X_tru[Z==0]), col="blue")
lines(density(X_tru[Z==1]),col="red")
...
################################################################################
#fit1 <- glm(S ~ X_tru + factor(Z), family = tweedie(var.power=1.5, link.power=0))
#fit1 <- glm( S ~  X_tru + factor(Z), family=poisson(link = "log") )
...
# cl_membership <- full.train.brazil$StateAb
# table(cl_membership)
# cl_membership <- as.numeric( factor(cl_membership) )
# table(cl_membership)

# cl_membership.test <- full.test.brazil$StateAb
# table(cl_membership.test)
# cl_membership.test <- as.numeric( factor(cl_membership.test) )
# table(cl_membership.test)
# J=27
...
pi_empirical = aggregate(x=Z, by=list(cl_membership.train), FUN=mean, na.rm= T)$x; pi_empirical
...
#> 1. Regular
for (j in 1:J) {
  Xj=X_tru[cl_membership.train==j]
  n_j=length(Xj)
...
X_bar = aggregate(x=X_tru, by=list(cl_membership.train), FUN=mean, na.rm= T)$x; X_bar
# ---------------------------------------------------------------- Perfect!!!!!!
...
for(j in 1:J) {
  ...
  Sj = S[cl_membership.train==j]
  matXj = matX_tru[cl_membership.train==j, ]
...
for(h in 1:n.train) {
  ...
  f0X[h] = f0x1(Z[h])*f0x2(X_tru[h], X_tru)  
  ...
  for(j in 1:M) {
    ...
    sumES[j] = 12*exp( sum(matX_tru[h,]*beta_samplej) - sig_samplej/2 )*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  }
...
for (r in 1:total_iter) {
  ...
  clusterout = clusterDPBasic(S, X_tru, Z, cl_membership.train,
  ...
  cl_membership.train = clusterout$cl_membership
  ...
  for(j in 1:J) {
    ...
    Sj = S[cl_membership.train==j]
    if(length(Sj)==1) {
      matXj = matrix( matX_tru[cl_membership.train==j, ], nrow=1 ) # It is a vector added to the matrix
    } else {
      matXj = matX_tru[cl_membership.train==j, ]                   # It is a regular matrix
    } 
  ...
  if(r > r_convergence) {
    ...
    list_cl[[r-r_convergence]] = cl_membership.train
  }
...
table(cl_membership.train) # final number of cl...
...
format( aggregate(log(S), by=list(cl_membership.train), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.DP.Avg.train), by=list(cl_membership.train), FUN=summary), scientific=FALSE)
  ...
  for(h in 1:n.test) {
    ...
    for(j in 1:J) {
      ...
      f_S.test[j] = dlogsknorm(s=S.test[h], x=c(1, X_tru.test[h], Z.test[h]), 
                               beta=betaparamclean[j,], sig2=sig2paramclean[j], xi=xiparamclean[j])
      #% compute pred.value
      E_value.test[j] = 12*exp(sum( c(1, X_tru.test[h], Z.test[h])*betaparamclean[j,]) - sig2paramclean[j]/2)*
        (1-pnorm(-xiparamclean[j]*sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
    }
...
# den.DP_df <- data.frame(density.test); head(den.DP_df,2) 
# log_val.DP_df <- data.frame(log(expval.test)); head(log_val.DP_df,2) 
# 
# hist(log_val.DP_df[,1], breaks=n.breaks.test) 
# hist(log_val.DP_df[,2], breaks=n.breaks.test) 
# hist(log_val.DP_df[,3], breaks=n.breaks.test)
# hist(log_val.DP_df[,4], breaks=n.breaks.test) 
# 
...
# long.den.DP <- den.DP_df %>% pivot_longer(everything())
# long.log_val.DP <- log_val.DP_df %>% pivot_longer(everything())
# long.den.DP %>% ggplot( aes(x=value, color=name, fill=name) ) + geom_density( alpha=0.3 )
# ggplot(long.log_val.DP, aes(x=value, y=name, group=name)) + geom_density_ridges(bandwidth=0.15) + xlim(6, 16)
### ???????????????????????????????????????????????????????????????????????????
...
SSPE.DP.LSN <- sum( (log(expS.DP.Avg.test) - log(S.test))^2 ); SSPE.DP.LSN    #
SAPE.DP.LSN <- sum( abs(log(expS.DP.Avg.test) - log(S.test)) ); SAPE.DP.LSN   #
lppd_EX_DP.S                                                                  #
...
#> Gaussian regression for initialize OUTCOME model parameter m0, SIG_b0
fit1.xx <- glm(S ~ Xstar + factor(Z), family = tweedie(var.power=1.5, link.power=0))
options(scipen = 999)
summary(fit1.xx)
...
# cl_membership.xx <- full.train.brazil$StateAb
# table(cl_membership.xx)
# cl_membership.xx <- as.numeric( factor(cl_membership.xx) )
# table(cl_membership.xx)
# 
# J=27
...
#% [Check!] empirical pi ? using "aggregate(.)": investigation by splitting the data into subset
pi_empirical = aggregate(x=Z, by=list(cl_membership.train.xx), FUN=mean, na.rm= T)$x; pi_empirical
...
#> 1. Regular
for (j in 1:J) {
  Xj=Xstar[cl_membership.train.xx==j]
  n_j=length(Xj)
...
#% empirical X_barF ? using "aggregate(.)": investigation by splitting the data into subset
X_bar.xx = aggregate(x=Xstar, by=list(cl_membership.train.xx), FUN=mean, na.rm= T)$x; X_bar.xx
# ---------------------------------------------------------------- Perfect!!!!!!
...
for(j in 1:J) {
  ...
  # subsetting by cluster
  Sj = S[cl_membership.train.xx==j]
  matXj = matXstar[cl_membership.train.xx==j, ]
...
for(h in 1:n.train) {
  ...
  ##> Analytical solution for f0(X, Z) for ### covariate model ###
  # : necessary parameters are supplied by pre-determined hyperparam values...
  f0X.xx[h] = f0x1(Z[h])*f0x2(Xstar[h], Xstar)  
  ...
  for(j in 1:M) {
    ...
    # 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
    sumES.xx[j] = 12*exp( sum(matXstar[h,]*beta_samplej) - sig_samplej/2 )*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  }
...
for (r in 1:total_iter) {
  ##############################################################################################################
  ##[1] cluster membership computation ----------------------------------------
  clusterout = clusterDPBasic(S, Xstar, Z, cl_membership.train.xx,
  ...
  cl_membership.train.xx = clusterout$cl_membership
  J = length(unique(cl_membership.train.xx))             #% in case, cluster scenario changes, reflecting them
  ...
  for(j in 1:J) {
    ...
    Sj = S[cl_membership.train.xx==j]
    if(length(Sj)==1) {
      matXj = matrix( matXstar[cl_membership.train.xx==j, ], nrow=1 ) # It is a vector added to the matrix
    } else {
      matXj = matXstar[cl_membership.train.xx==j, ]                   # It is a regular matrix
    } # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Q.David
  ...
  if(r > r_convergence) {
    ...
    list_cl.xx[[r-r_convergence]] = cl_membership.train.xx
  }
  print(paste("r=",r)) #% iteration progress
}
...
#> summary(log(expS.DP.train))
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#7.673   9.891  10.687  11.016  11.836  16.769


format( aggregate(log(S), by=list(cl_membership.train.xx), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.DP.Avg.train.xx), by=list(cl_membership.train.xx), FUN=summary), scientific=FALSE)
...
for(h in 1:n.test) {
  ...
  for(j in 1:M) {
    ...
    sumS.xx[j] = dlogsknorm( s=S.test[h], x=matXstar.test[h,], beta=beta_samplej, sig2=sig_samplej, xi=xi_samplej )* 
      ...
      dt(x=xi_samplej, df=nu0.xx)                                 # to joint xi
    # 20*exp( betaparamclean[j,1] + betaparamclean[j,2]*cleanx + betaparamclean[j,3]*Z[i] - sig2paramclean[j]/2)*( 1-pnorm(-xiparamclean[j]*sqrt(sig2paramclean[j])/sqrt(xiparamclean[j]^2+1)))
    sumES.xx[j] = 12*exp( sum(matXstar.test[h,]*beta_samplej) - sig_samplej/2 )*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  }
...
SSPE.DP.LSN.xx <- sum( (log(expS.DP.Avg.test.xx) - log(S.test))^2 ); SSPE.DP.LSN.xx   #273.266
SAPE.DP.LSN.xx <- sum( abs(log(expS.DP.Avg.test.xx) - log(S.test)) ); SAPE.DP.LSN.xx  #199.3943
...
#> Gaussian regression for initialize OUTCOME model parameter m0, SIG_b0
fit1.st <- glm(S ~ Xstar + factor(Z), family = tweedie(var.power=1.5, link.power=0))
...
# cl_membership.st <- full.train.brazil$StateAb
# table(cl_membership.st)
# cl_membership.st <- as.numeric( factor(cl_membership.st) )
# table(cl_membership.st)
# 
# J=27
...
for (j in 1:J) {
  Xstar_j=Xstar[cl_membership.train.st==j]
  Zj = Z[cl_membership.train.st==j]
  nj=length(Xstar_j)
...
#% [Check!] empirical kappa, lambda ? using "aggregate(.)": investigation by splitting the data into subset
kappa_empirical=rbind(coef(lm(Xstar[cl_membership.train.st==1] ~ Z[cl_membership.train.st==1])),
                      coef(lm(Xstar[cl_membership.train.st==2] ~ Z[cl_membership.train.st==2])),
                      coef(lm(Xstar[cl_membership.train.st==3] ~ Z[cl_membership.train.st==3]))); kappa_empirical

lambda_empirical = aggregate(x=as.numeric(Xstar), by=list(cl_membership.train.st), FUN=var, na.rm=T)$x; lambda_empirical
...
for(j in 1:J) {
  ...
  # subsetting by cluster
  Sj = S[cl_membership.train.st==j]
  matXj = matXstar[cl_membership.train.st==j, ]
...
for(h in 1:n.train) {
  ...
  for(j in 1:M) {
    ...
    sumES.st[j] = 0.2*exp(sum(matXstar[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
...
n_j.st = table(cl_membership.train.st)
...
for (r in 1:total_iter) {
  ...
  clusterout = clusterDP(S, Xstar, Z, cl_membership.train.st,
  ...
  cl_membership.train.st = clusterout$cl_membership
  J = length(unique(cl_membership.train.st))                  #% in case, cluster scenario changes, reflecting them
  ...
  for (j in 1:J) {
    ...
    while(difference1 < 0 | difference2 < 0 | difference3 < 0) {
      ...
      #>>> subsetting by cluster
      S_j = S[cl_membership.train.st==j]                                        # outcome
      matX_j.st = matrix(matXstar[cl_membership.train.st==j, ], nrow=length(S_j))   # covariate
  ...
  if(r > r_convergence) {
    ...
    list_cl.st[[r-r_convergence]] = cl_membership.train.st
...
lppd_EX_DP.S.st = mean( loglikelihood.st ); lppd_EX_DP.S.st # -21604.54
# crucial 2000
...
format( aggregate(log(S), by=list(cl_membership.train.st), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.DP.Avg.train.st), by=list(cl_membership.train.st), FUN=summary), scientific=FALSE)
...
for(h in 1:n.test) {
  ...
  for(j in 1:M) {
    ...
    sumS.st[j] = 
      ...
      dinvgamma(x = sig_samplej, shape = u0.st, scale = v0.st)*          # to joint sig2
      dt(x = xi_samplej, df = nu0.st)                                 # to joint xi
    sumES.st[j] = 10*exp(sum(matXstar.test[h,]*beta_samplej) - sig_samplej/2)*(1-pnorm(-xi_samplej*sqrt(sig_samplej)/sqrt(xi_samplej^2+1)))
  ...
  for(h in 1:n.test) {
    ...
    for(j in 1:J) {
      ...
      f_S.test.st[j] = dlogsknorm(s=S.test[h], x=c(1, Xstar.test[h], Z.test[h]), 
                                  beta=betaparamclean.st[j,], sig2=sig2paramclean.st[j], xi=xiparamclean.st[j])
      #% compute pred.value
      E_value.test.st[j] = 10*exp(sum( c(1, Xstar.test[h], Z.test[h])*betaparamclean.st[j,])-sig2paramclean.st[j]/2)*
        (1-pnorm(-xiparamclean.st[j]*sqrt(sig2paramclean.st[j])/sqrt(xiparamclean.st[j]^2+1)))
    }
...
SSPE.DP.LSN.st <- sum( (log(expS.DP.Avg.test.st) - log(S.test))^2 ); SSPE.DP.LSN.st   #
SAPE.DP.LSN.st <- sum( abs(log(expS.DP.Avg.test.st) - log(S.test)) ); SAPE.DP.LSN.st  #
...
# cl_membership.HB <- full.train.brazil$StateAb
# table(cl_membership.HB)
# cl_membership.HB <- as.numeric( factor(cl_membership.HB) )
# table(cl_membership.HB)
# 
# cl_membership.HB.test <- full.test.brazil$StateAb
# table(cl_membership.HB.test)
# cl_membership.HB.test <- as.numeric( factor(cl_membership.HB.test) )
# table(cl_membership.HB.test)
# J=27

A = c("AC", "AP", "AM", "PA", "RO", "RR", "TO") #Norte
B = c("AL", "BA", "CE", "MA", "PB", "PE", "PI", "RN", "SE") #Nordeste
C = c("DF", "GO", "MT", "MS") #Centro-Oeste
D = c("ES", "MG", "RJ", "SP") #Sudeste
E = c("PR","RS","SC") #Sul


full.train.brazil$Region <- NA

for (i in (1:n.train) ) {
  if (full.train.brazil$StateAb[i] %in% A) {
    full.train.brazil$Region[i] <- "A"
  }
  else if (full.train.brazil$StateAb[i] %in% B) {
    full.train.brazil$Region[i] <- "B"
  }
  else if (full.train.brazil$StateAb[i] %in% C) {
    full.train.brazil$Region[i] <- "C"
  }
  else if (full.train.brazil$StateAb[i] %in% D) {
    full.train.brazil$Region[i] <- "D"
  }
  else if (full.train.brazil$StateAb[i] %in% E) {
    full.train.brazil$Region[i] <- "E"
  }
}
...
full.test.brazil$Region <- NA
for (i in (1:n.train) ) {
  if (full.test.brazil$StateAb[i] %in% A) {
    full.test.brazil$Region[i] <- "A"
  }
  else if (full.test.brazil$StateAb[i] %in% B) {
    full.test.brazil$Region[i] <- "B"
  }
  else if (full.test.brazil$StateAb[i] %in% C) {
    full.test.brazil$Region[i] <- "C"
  }
  else if (full.test.brazil$StateAb[i] %in% D) {
    full.test.brazil$Region[i] <- "D"
  }
  else if (full.test.brazil$StateAb[i] %in% E) {
    full.test.brazil$Region[i] <- "E"
  }
}

head(full.test.brazil)


cl_membership.HB.train <- full.train.brazil$Region
table(cl_membership.HB.train)
cl_membership.HB.train <- as.numeric( factor(cl_membership.HB.train) )
table(cl_membership.HB.train)

cl_membership.HB.test <- full.test.brazil$Region
table(cl_membership.HB.test)
cl_membership.HB.test <- as.numeric( factor(cl_membership.HB.test) )
table(cl_membership.HB.test)
...
#> Gaussian regression for initialize OUTCOME model parameter m0, SIG_b0
fit1.HB.st <- glm(S ~ Xstar + factor(Z), family = tweedie(var.power=1.5, link.power=0))
...
################################################################################
J.HB = length(table(cl_membership.HB.train))
set.seed(1)
...
#% [Check!] empirical pi ? using "aggregate(.)": investigation by splitting the data into subset
pi_empirical = aggregate(x=Z, by=list(cl_membership.HB.train), FUN=mean, na.rm= T)$x; pi_empirical
...
for (j in 1:J.HB) {
  Xstar_j=Xstar[cl_membership.HB.train==j]
  Zj = Z[cl_membership.HB.train==j]
...
#% [Check!] empirical kappa, lambda ? using "aggregate(.)": investigation by splitting the data into subset
kappa_empirical=rbind(coef(lm(Xstar[cl_membership.HB.train==1] ~ Z[cl_membership.HB.train==1])),
                      coef(lm(Xstar[cl_membership.HB.train==2] ~ Z[cl_membership.HB.train==2])),
                      coef(lm(Xstar[cl_membership.HB.train==3] ~ Z[cl_membership.HB.train==3]))); kappa_empirical

lambda_empirical.HB.st = aggregate(x=as.numeric(Xstar), by=list(cl_membership.HB.train), FUN=var, na.rm=T)$x; lambda_empirical.HB.st
...
for(j in 1:J.HB) {
  ...
  # subsetting by cluster
  Sj = S[cl_membership.HB.train==j]
  #logSj = logS[cl_membership.HB.train==j]
  matXj = matX_tru[cl_membership.HB.train==j, ]
...
n_j.HB.st = table(cl_membership.HB.train)
...
for (r in 1:total_iter) {
  ...
  for(j in 1:J.HB) {
    ...
    Xstar_j=Xstar[cl_membership.HB.train==j]
    Z_j = Z[cl_membership.HB.train==j]
    ...
    while( difference1 < 0 | difference2 < 0 | difference3 < 0) {
      ...
      #>>> subsetting by cluster
      S_j = S[cl_membership.HB.train==j]                  #outcome
      matX_j.HB.st = matXstar[cl_membership.HB.train==j, ] #covariate
  ...
  for(i in 1:n.train) {
    ...
    j = cl_membership.HB.train[i] # membership ID
...
format( aggregate(log(S), by=list(cl_membership.HB.train), FUN=summary), scientific=FALSE)
format( aggregate(log(expS.HB.train.st), by=list(cl_membership.HB.train), FUN=summary), scientific=FALSE)
...
for(j in 1:J.HB) {
  hist(log(S)[cl_membership.HB.train==j], freq=FALSE, breaks=50, col="white", xlim=c(5,15), ylim=c(0, 2.8),
       xlab = paste("Cluster", j))
  #lines(density(log(expS.HB.train)[cl_membership.HB==j]), col="red", lwd=2)     # clean 
  #lines(density(log(EX_S.train.dirty)[cl_membership.HB==j]), col="black", lty = "dotted") # dirty
  lines(density(log(expS.HB.train.st)[cl_membership.HB.train==j]), col="blue", lwd=1) # dirty with correction
  
  if (j == 1) {
    legend("topright", legend = c("Clean", "Correction"), #, "Correction Medium", "Correction Small"),
           col = c("red", "blue"), #, "darkmagenta", "darkgreen"), 
           lty = c(1, 2), 
           lwd = c(3, 2), 
           cex = 0.4
    )
...
##> Modelrisk
expS.DP.Avg.test.xx
SSPE.DP.LSN.xx <- sum( (log(expS.DP.Avg.test.xx) - log(S.test))^2 ); SSPE.DP.LSN.xx   #
SAPE.DP.LSN.xx <- sum( abs(log(expS.DP.Avg.test.xx) - log(S.test)) ); SAPE.DP.LSN.xx  #
#expS.HB.test.xx
...
SAPE.DP.LSN.st <- sum( abs(log(expS.DP.Avg.test.st) - log(S.test)) ); SAPE.DP.LSN.st  #
expS.HB.test.st
SSPE.HB.LSN.st <- sum( (log(expS.HB.test.st) - log(S.test))^2 ); SSPE.HB.LSN.st   # 
SAPE.HB.LSN.st <- sum( abs(log(expS.HB.test.st) - log(S.test)) ); SAPE.HB.LSN.st  # 
